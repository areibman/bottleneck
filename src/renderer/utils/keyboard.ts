...
export function setupKeyboardShortcuts() {
  if (!window.electron) {
    console.warn('window.electron not available, skipping keyboard shortcuts setup');
    return () => {};
  }

  const handleKeyDown = (e: KeyboardEvent) => {
    const { toggleSidebar, toggleRightPanel, toggleCommandPalette, toggleKeyboardShortcuts } = useUIStore.getState();
