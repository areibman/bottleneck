...
import { UncontrolledMarkdownEditor, UncontrolledMarkdownEditorRef } from '../components/UncontrolledMarkdownEditor';
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      if (!token || token === 'dev-token') {
        ...
        const api = new GitHubAPI(token);
        const comment = await api.createIssueComment(owner, repo, parseInt(number), commentText);
  ...
  const handleUpdateIssue = async () => {
    ...
    try {
      if (!token || token === 'dev-token') {
        ...
        const api = new GitHubAPI(token);
        const updatedIssue = await api.updateIssue(owner, repo, parseInt(number), issueText);
  ...
  const handleUpdateComment = async (commentId: number, commentText?: string) => {
    ...
    try {
      if (!token || token === 'dev-token') {
        ...
        const api = new GitHubAPI(token);
        const updatedComment = await api.updateIssueComment(owner, repo, commentId, text);
        setComments(comments.map(c => 
                ...
                <div>
                  ...
                  {issue.assignees.length > 0 ? (
                    issue.assignees.map(a => (
                      <div key={a.login} className="flex items-center space-x-2 mb-1">
