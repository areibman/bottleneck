...
export const Terminal = React.memo(function Terminal({ className, onResize }: TerminalProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (screenElement) {
        (screenElement as HTMLElement).style.cssText = 'padding: 10px 10px 10px 10px !important;'
        console.log('[Terminal] Applied 10px padding to all sides of terminal screen')
      }
        ...
        if (window.electron?.terminal) {
          try {
            ...
            if (window.electron?.settings) {
              const settingsResult = await window.electron.settings.get('cloneLocation')
              defaultDir = settingsResult?.value || undefined
            }
            
            // Spawn terminal in clone location or default
            const spawnResult = await window.electron.terminal.spawn(defaultDir)
            if (!spawnResult.success) {
              console.error('Failed to spawn terminal:', spawnResult.error)
              terminal.write(`\x1b[31mTerminal startup failed: ${spawnResult.error}\x1b[0m\r\n`)
            } else {
              console.log('[Terminal] Process spawned successfully in:', defaultDir || 'default directory')
            ...
            terminal.write(`\x1b[31mTerminal startup error: ${(error as Error).message}\x1b[0m\r\n`)
          }
        }
      } catch (error) {
  ...
  }, [settings.terminalFontSize, settings.terminalFontFamily, isReady, debouncedFit])
