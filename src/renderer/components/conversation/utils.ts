  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inTable && line.startsWith('|') && !line.includes('---')) {
      ...
      if (cells.length >= 4) {
        ...
        else if (deploymentCell.toLowerCase().includes('ignored')) status = 'Ignored';
        else if (deploymentCell.toLowerCase().includes('building')) status = 'Building';
        else if (deploymentCell.toLowerCase().includes('error')) status = 'Error';
        else if (deploymentCell.toLowerCase().includes('canceled')) status = 'Canceled';
