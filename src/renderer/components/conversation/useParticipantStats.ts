...
export function useParticipantStats(pr: PullRequest, comments: Comment[], reviews: Review[]): ParticipantStat[] {
  return useMemo(() => {
    const stats = new Map<string, ParticipantStat>();
    
    // Add author
    const isAuthorBot = pr.user.login.includes('[bot]') || pr.user.login.includes('bot');
    ...
    comments.forEach(comment => {
      ...
      const isBot = login.includes('[bot]') || login.includes('bot') || login.includes('vercel');
      ...
      if (latestComment && latestComment.body) {
        ...
        if (isBot || latestComment.body.toLowerCase().includes('deploy') || 
