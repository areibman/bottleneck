...
export class GitOperations {
  ...
  async checkout(repoPath: string, branch: string, create: boolean = false): Promise<void> {
    const git = this.getGit(repoPath);
  ...
  async getDiff(repoPath: string, base: string, head: string): Promise<DiffResult> {
    const git = this.getGit(repoPath);
  ...
  async getFileDiff(repoPath: string, file: string, base: string, head: string): Promise<string> {
    const git = this.getGit(repoPath);
  ...
  async push(repoPath: string, branch?: string, force: boolean = false): Promise<void> {
    const git = this.getGit(repoPath);
    const args = ['origin'];
